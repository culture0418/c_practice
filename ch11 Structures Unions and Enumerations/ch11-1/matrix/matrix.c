...
void add_mat(const struct mat *m1, const struct mat *m2, struct mat *result) //! 分別是 m1, m2, result
{
    result->row = m1->row;
    result->col = m1->col;
...
void mul_mat(const struct mat *m1, const struct mat *m2, struct mat *result)
{
    ...
    for (int row = 0; row < result->row; row++)
    {
        ...
        {
            result->value[row][col] = 0;
            for (int i = 0; i < m1->col; i++)
            {
                result->value[row][col] += m1->value[row][i] * m2->value[i][col];
            }
        }
    }
}